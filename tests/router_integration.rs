...
use reqwest::Client;
use routiium::{
    router_client::{CachedRouterClient, HttpRouterClient, HttpRouterConfig, RouterClient},
...
async fn router_strict_mode_surfaces_error() {
    ...
    assert_eq!(
        resp.status().as_u16(),
        RouterStatusCode::BAD_GATEWAY.as_u16()
    );
    let body = test::read_body(resp).await;
    let body_text = String::from_utf8_lossy(&body);
    assert!(
        body_text.contains("Router error"),
        "expected router error message, got {body_text}"
    );
    clear_router_env();
}
