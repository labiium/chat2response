# Chat2Response Router Contract — Schema 1.1

This document defines the Router interoperability contract for Chat2Response following the
...
| Item | Requirement |
|------|-------------|
| Base protocol | HTTP/1.1 or HTTP/2 with keep-alive (Chat2Response pools 10 idle connections). |
...
- `request_id` is generated by the client (Chat2Response uses a short UUID prefixed with `req_`).
...
- `ttl_ms` defines the soft lifetime for reuse. Chat2Response also honours `valid_until` when provided.
- `freeze_key` lets routers invalidate a single cached plan without altering revisions.
- `stickiness.plan_token` (with optional `max_turns`/`expires_at`) instructs Chat2Response to reuse the plan for future turns by sending the token back in `RouteRequest.plan_token`.
...
Routers should set `ETag` (quoted) and honour `If-None-Match`. Chat2Response polls the catalog on startup and every 60 seconds.
...
Chat2Response propagates these headers to client responses when `CHAT2R_EXPOSE_ROUTE_HEADERS=1` is set.
...
2. Chat2Response stores the token alongside the plan for the configured TTL/turn count.
...
- Chat2Response still accepts legacy 1.0 fields but always emits 1.1 payloads.
...
By adhering to Schema 1.1 your Router gains precise accounting, better traceability, and forward-compatible caching behaviour while maintaining full backwards compatibility with earlier Chat2Response deployments.
